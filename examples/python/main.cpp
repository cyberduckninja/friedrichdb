...
class data_set {
public:
    data_set(pybind11::object collections, context* ctx = nullptr ) {
        for(auto&i:collections){
            data_set_.emplace_back(to_json_impl(i));
        }
...
private:
    ...
    field to_json_impl(const py::handle &obj) {
        if (obj.is_none()) {
            return nullptr;
